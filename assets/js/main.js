...
if (form) {
  ...
  form.addEventListener('submit', e => {
    ...
    // ── Email Brevo via send-email Edge Function ──
    fetch('https://xrxlteycmcmztdqscfeq.supabase.co/functions/v1/send-email', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      ...
      body: JSON.stringify({
        ...
        record: {
          ...
          etape:   data.etape   || '',
          projet:  data.projet  || '',
          rentree: data.rentree || '',
        }
      })
    }).catch(err => console.error('[Brevo lead email]', err));
